...
void code(std::string expr, int codeType) {
    //codeTypes:
    //ID        : 1
    //expr      : 2
    //term      : 3
    ...
    if(codeType == 1) {
        ...
        //Count how many expr subcalls there are
        int exprCalls = 0;
        //Check if dcls is empty - if it is we can skip
        std::string extractedExps;
        std::stringstream paramStream(nextLine);
        paramStream >> extractedExps; // gets exps
        std::cout << "next arg: " << extractedExps << std::endl;
        paramStream >> extractedExps; // gets first arg
        std::cout << "next arga: " << extractedExps << std::endl;
        while(!extractedExps.compare("expr")) {
            exprCalls++;
            std::getline(std::cin, nextLine);
            std::stringstream paramStream2(nextLine);
            paramStream2 >> extractedExps; // gets exps
            paramStream2 >> extractedExps; // gets first arg
        }
        std::cout << "there are " << exprCalls << " expressions " <<std::endl;
        for(int i = 0; i < exprCalls; i++) {
            code("term", 3);  
            push(3);
            code("term", 3);
            pop(5);
        ...
        //current line has to be expr term
        // std::stringstream paramStream2(nextLine);
        // paramStream2 >> extractedExps; // gets exps
        // paramStream2 >> extractedExps; // gets first arg
        ...
        // term → factor 
        std::cout << "code for term" << std::endl;
        std::string nextLine;
        std::getline(std::cin, nextLine);
        std::string extractedExps;
        std::stringstream paramStream(nextLine);
        ...
        paramStream >> extractedExps; // gets first arg
        std::cout << "next arga: " << extractedExps << std::endl;
        if(!extractedExps.compare("factor")) {
            code("factor", 4);
        }
    } else if(codeType == 4) {
        // factor → NUM  
        ...
        std::string nextLine;
        std::cout << "code for factor" << std::endl;
        std::getline(std::cin, nextLine);
        std::string extractedExps;
        std::stringstream paramStream(nextLine);
        paramStream >> extractedExps; // gets factor
        std::cout << "next arg: " << extractedExps << std::endl;
        paramStream >> extractedExps; // gets first arg
        std::cout << "next arga: " << extractedExps << std::endl;
        if(!extractedExps.compare("ID")) {
            std::getline(std::cin, nextLine);
            code(nextLine, 1);
        } else if(!extractedExps.compare("LPAREN")) {
            code("expr", 2);
            std::getline(std::cin, nextLine);//flush RPAREN
        } else if(!extractedExps.compare("NUM")) {
            std::getline(std::cin, nextLine);
            std::stringstream paramStream2(nextLine);
            paramStream2 >> extractedExps; // gets NUM
            paramStream2 >> extractedExps; //gets value
            std::cout << "lis $3\n.word " << extractedExps << std::endl;
            // push(3);
        }
    }
}
