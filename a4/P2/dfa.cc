...
//   }
  ...
  while(in >> s) {
    ...
    if (s == EMPTY) {
        ...
        bool is_accepting = std::count(std::begin(accepting_states), std::end(accepting_states), initial_state) > 0;
        if (is_accepting) {
        ...
        for (char c : s) {
            ...
            current_transition.second = c;
            auto it = transitions.find(current_transition);
            if(it == transitions.end()) {
                // std::cout << "transition from " << current_state << " with " << c << " doesnt exist" << std::endl;
